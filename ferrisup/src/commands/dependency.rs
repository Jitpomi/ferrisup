...
pub fn add_dependencies(args: AddArgs) -> Result<()> {
    ...
    if !cargo_toml_path.exists() {
        return Err(anyhow::anyhow!(
            "No Cargo.toml found in the specified directory. Are you sure this is a Rust project?"
        ));
    }
    ...
    let section_to_check = if args.dev {
        "dependencies"
    } else {
        "dev-dependencies"
    };
    let target_section = if args.dev {
        "dev-dependencies"
    } else {
        "dependencies"
    };
    let section_flag = if section_to_check == "dev-dependencies" {
        "--dev"
    } else {
        ""
    };
        ...
        if let Some(deps_table) = deps_table.as_table() {
            for dependency in &dependencies {
                if deps_table.contains_key(dependency) {
                    ...
                    let output = remove_cmd.output().context(format!(
                        "Failed to remove dependency {} from {}",
                        dependency, section_to_check
                    ))?;
                    ...
                    if !output.status.success() {
                        ...
                        println!(
                            ...
                            format!(
                                "Failed to remove dependency {} from {}: {}",
                                dependency, section_to_check, error
                            )
                            .yellow()
                        );
                    }
...
fn suggest_features_interactive(dependency: &str) -> Result<Vec<String>> {
    ...
    if let Some(suggested_features) = get_popular_features(dependency) {
        ...
        let selections = MultiSelect::new().items(&suggested_features).interact()?;
...
pub fn analyze_dependencies(args: AnalyzeArgs) -> Result<()> {
    ...
    if !audit_installed {
        println!(
            "{}",
            "cargo-audit is not installed. It's recommended for security analysis.".yellow()
        );
        if Confirm::new()
            ...
            .interact()?
        {
            ...
            if !output.success() {
                println!(
                    ...
                    "Failed to install cargo-audit:".red()
                );
            } else {
            ...
            .unwrap_or(false)
    {
        ...
        if !audit_output.success() {
            println!(
                "{}",
                "Security vulnerabilities found in your dependencies. Please review and update."
                    .yellow()
            );
        }
    }
