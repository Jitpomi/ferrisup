...
pub fn fix_component_imports(component_dir: &Path, component_name: &str) -> Result<()> {
    println!(
        "{}",
        format!("Fixing imports in component: {}", component_name).blue()
    );
    ...
    if cargo_toml_path.exists() {
        let cargo_content =
            fs::read_to_string(&cargo_toml_path).context("Failed to read component Cargo.toml")?;
                    ...
                    if current_name != new_package_name {
                        *name = value(new_package_name.clone());
                        ...
                        println!(
                            "{}",
                            format!(
                                "  Updated package name in Cargo.toml to: {}",
                                new_package_name
                            )
                            .blue()
                        );
        ...
        .filter(|e| e.file_type().is_file())
    {
        ...
        if let Some(ext) = path.extension() {
            if ext == "rs" {
                ...
                let re_component = match Regex::new(&format!(
                    r"use\s+{}(::|\s+)",
                ...
                let re_unknown = match Regex::new(&format!(
                    r"use\s+unknown_{}(::|\s+)",
                    regex::escape(component_name)
                )) {
                    Ok(r) => r,
                ...
                let updated_content1 =
                    re_component.replace_all(&content, format!("use {}{}", new_package_name, "$1"));
                let updated_content2 = re_unknown.replace_all(
                    &updated_content1,
                    format!("use {}{}", new_package_name, "$1"),
                );
