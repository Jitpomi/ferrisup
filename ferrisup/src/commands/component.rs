...
fn add_component(project_dir: &Path, component_type: Option<&str>) -> Result<()> {
    ...
    let result = if is_workspace {
        ...
        crate::commands::transform::component::add_component(project_dir)
    } else {
        // For non-workspace projects - shows only module-compatible components
        crate::commands::transform::component::add_component_without_workspace(project_dir)
    };
...
fn remove_component(project_dir: &Path, component_type: Option<&str>) -> Result<()> {
    ...
    let component_path = if let Some(ctype) = component_type {
        ...
        let matching: Vec<String> = components
            .iter()
    ...
    if is_workspace {
        ...
        if !usage_summary.is_empty() {
            println!(
                "{}",
                "⚠️  Component is being used in other components:".yellow()
            );
            println!();
            ...
            println!(
                "{}",
                "You must manually remove these references before removing the component.".yellow()
            );
    ...
    fs::remove_dir_all(&full_path).context(format!("Failed to remove {}", full_path.display()))?;

    println!(
        "{} {}",
        "Successfully removed component:".green(),
        component_path
    );
...
fn remove_component_from_metadata(project_dir: &Path, component_name: &str) -> Result<()> {
    ...
    if metadata_doc.contains_key(&component_key) {
        ...
        println!(
            "{} {}",
            "Updated".green(),
            "metadata.toml to remove component".cyan()
        );
    ...
    if updated {
        ...
        println!(
            ...
            format!(
                "'{}' dependency from {}",
                dependency_name,
                cargo_path.file_name().unwrap().to_string_lossy()
            )
            .cyan()
        );
    }
...
fn check_component_usage(
    ...
    println!(
        "{} {}",
        "Checking for usage of".blue(),
        component_to_check.cyan()
    );
    ...
    for component in all_components {
        ...
        if !src_dir.exists() || !src_dir.is_dir() {
            println!(
                "{} {}",
                "No src directory found in".yellow(),
                component.cyan()
            );
            continue;
        }
            ...
            for pattern in &import_patterns {
                if trimmed.contains(pattern) {
                    ...
                    let file_relative = file_path
                        .strip_prefix(project_dir.join(component_name))
                        .unwrap_or(file_path)
                        .to_string_lossy()
                        .to_string();
                    ...
                    if !file_entries
                        .iter()
                        .any(|(num, content)| *num == entry.0 && content == &entry.1)
                    {
...
fn update_workspace_members_after_removal(
    project_dir: &Path,
