...
pub fn execute(path: Option<&str>) -> Result<()> {
    ...
    if !is_cargo_unused_features_installed() {
        ...
        match install_result {
            Ok(status) if status.success() => {
                println!(
                    "{}",
                    "Successfully installed cargo-unused-features.".green()
                );
            }
            Ok(_) => {
                return Err(anyhow::anyhow!(
                    "Failed to install cargo-unused-features. Please install it manually with 'cargo install cargo-unused-features'"
                ));
            }
            Err(e) => {
                return Err(anyhow::anyhow!(
                    "Error installing cargo-unused-features: {}",
                    e
                ));
            }
        }
    ...
    if output.status.success() {
        ...
        if stderr.contains("Prune") {
            ...
            for line in log_lines {
                if line.contains("==== Dependency") {
                    ...
                    if let Some(feature_name) = line.split("'").nth(1) {
            ...
            println!(
                "- Review the unused features and consider removing them from your Cargo.toml"
            );
        ...
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(anyhow::anyhow!(
            "Error running cargo-unused-features: {}",
            stderr
        ));
    }
...
fn is_cargo_unused_features_installed() -> bool {
    let output = Command::new("unused-features").arg("--help").output();
