...
pub fn make_shared_component_accessible(project_dir: &Path, component_name: &str) -> Result<()> {
    ...
    // Now find all other component directories and add the shared component as a dependency
                ...
                if let Some(members_array) = members.as_array() {
                    for member in members_array {
                        if let Some(member_str) = member.as_str() {
                            ...
                            // Add the shared component as a dependency to this component
...
// Helper function to add a shared dependency to a component's Cargo.toml
fn add_shared_dependency_to_component(cargo_path: &Path, shared_component: &str) -> Result<()> {
    let cargo_content = fs::read_to_string(cargo_path)?;
    ...
    if let Some(dependencies) = doc.get_mut("dependencies") {
        if let Some(deps_table) = dependencies.as_table_mut() {
            ...
            if deps_table.get(shared_component).is_none() {
                // Create a proper TOML table for the dependency
                let mut dep_table = toml_edit::Table::new();
                dep_table.insert("path", value(format!("../{}", shared_component)));
                deps_table.insert(shared_component, toml_edit::Item::Table(dep_table));
                
                println!("{} {}", "Added".green(), 
                    format!("'{}' as dependency to {}", shared_component, cargo_path.parent().unwrap().file_name().unwrap().to_string_lossy()).cyan());
...
// Helper function to add a shared workspace dependency to a component's Cargo.toml
