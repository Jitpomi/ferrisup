        ...
        if let Some(extension) = file.extension() {
            if extension == "rs" {
                if let Ok(content) = fs::read_to_string(file) {
                    ...
                    let updated_content_str =
                        crate_regex.replace_all(&updated_content, format!("{}", new_name).as_str());
                ...
                if let Some(members_array) = members.as_array_mut() {
                    ...
                    if !component_exists {
                        ...
                        println!(
                            "{} {}",
                            "Added".green(),
                            format!("'{}' to workspace members", component_name).cyan()
                        );
                    }
                }
            }
...
pub fn make_shared_component_accessible(project_dir: &Path, component_name: &str) -> Result<()> {
            ...
            if let Some(deps) = workspace_table.get_mut("dependencies") {
                if let Some(deps_table) = deps.as_table_mut() {
                    ...
                    let setup_for_publishing = dialoguer::Confirm::new()
                        .with_prompt(format!(
                            "Would you like to set up '{}' for publishing to crates.io?",
                            component_name
                        ))
                        .default(true)
                        .interact()
                    ...
                    if setup_for_publishing {
                        ...
                        println!(
                            ...
                            "Added".green(),
                            format!(
                                "'{}' to workspace.dependencies with path and version",
                                component_name
                            )
                            .cyan()
                        );
                        ...
                        println!(
                            "{} {}",
                            "Added".green(),
                            format!("'{}' to workspace.dependencies with path", component_name)
                                .cyan()
                        );
                    }
                }
                ...
                if let Some(members_array) = members.as_array() {
                    for member in members_array {
                        if let Some(member_str) = member.as_str() {
                            ...
                            let component_cargo_path =
                                project_dir.join(member_str).join("Cargo.toml");
                            if component_cargo_path.exists() {
                                if setup_for_publishing {
                                    // Add as a workspace dependency (will use the version from workspace)
                                    add_shared_workspace_dependency_to_component(
                                        &component_cargo_path,
                                        component_name,
                                    )?;
                                    println!(
                                        "{} {}",
                                        "Added".green(),
                                        format!(
                                            "'{}' as a workspace dependency to component '{}'",
                                            component_name, member_str
                                        )
                                        .cyan()
                                    );
                                } else {
    ...
    if setup_for_publishing {
        ...
        println!(
            "{} {}",
            "•".yellow(),
            "Your shared component is now set up for publishing to crates.io".blue()
        );
        ...
        println!(
            ...
            "•".yellow(),
            format!(
                "Run 'cd {} && cargo publish' to publish your shared component",
                component_name
            )
            .blue()
        );
...
fn add_shared_workspace_dependency_to_component(
    cargo_path: &Path,
...
pub fn update_root_file_references(
    project_dir: &Path,
    component_name: &str,
