...
fn validate_project_name(name: &str) -> Result<()> {
    ...
    if !valid_first || !valid_rest {
        return Err(anyhow!(
            "Invalid project name '{}'. Use an ASCII letter first, followed only by letters, numbers, '-' or '_'",
            name
        ));
    }
...
pub fn execute(
    ...
    let mut template = match component_type {
        ...
        None => {
            if no_interactive {
                return Err(anyhow!(
                    "Component type is required in non-interactive mode"
                ));
            }

            let templates_with_desc = templates::list_templates()?;
            let mut templates: Vec<&str> = templates_with_desc
                .iter()
            ...
            let template_items: Vec<String> = templates
                .iter()
    ...
    if template == "server" {
        ...
        let framework_selected = if let Some(fw) = framework {
            if framework_options.contains(&fw) {
                ...
                println!(
                    "Warning: Provided framework '{}' is not valid for server components",
                    fw
                );
        ...
        let framework_dir =
            PathBuf::from(format!("{}/server/{}", template_root, framework_selected));

        if !framework_dir.exists() {
            return Err(anyhow::anyhow!(
                "Could not find template directory for {} framework",
                framework_selected
            ));
        }
        ...
        if cargo_toml_path.exists() {
            ...
            return Err(anyhow::anyhow!(
                "Could not find Cargo.toml.template for {} framework",
                framework_selected
            ));
        }
        ...
        if src_dir.exists() {
            ...
            for entry in fs::read_dir(&src_dir)? {
                ...
                if source_path.is_dir() {
                    ...
                    let rendered = match handlebars.render_template(&content, &template_vars) {
                        ...
                        Err(e) => {
                            println!(
                                "Warning: Template parsing error in {}: {}",
            ...
            return Err(anyhow::anyhow!(
                "Could not find src directory for {} framework",
                framework_selected
            ));
        }
        ...
        for entry in fs::read_dir(&framework_dir)? {
            ...
            if file_name_str == "src"
                || file_name_str == "Cargo.toml.template"
                || file_name_str == "README.md"
                || file_name_str == "template.json"
            {
                continue;
            }
        ...
        let template_json_path = PathBuf::from(format!(
            "{}/templates/server/template.json",
        ...
        if template_json_path.exists() {
            ...
            println!("\n🚀 Next steps:");
            if let Some(next_steps) = template_config
                .get(&next_steps_key)
        ...
        let selected_provider = if let Some(prov) = provider {
            if providers.contains(&prov) {
                println!(
                    "Using {} as the cloud provider for your serverless function",
                    prov
                );
                prov.to_string()
            } else {
                println!(
                    "Warning: Provided provider '{}' is not valid for serverless components",
                    prov
                );
            ...
            let selection = Select::new()
                .with_prompt(
                    "Which cloud provider would you like to target for your serverless function?",
                )
                .items(&providers)
                .default(0)
                .interact()?;
        ...
        let providers = ["aws", "gcp", "azure", "vercel", "netlify"];
        let template_json_path = PathBuf::from(format!(
            "{}/templates/serverless/template.json",
            env!("CARGO_MANIFEST_DIR")
        ));
        ...
        let _selected_provider =
            if let Some(vars) = template_json.get("variables").and_then(|v| v.as_object()) {
            ...
            if let Some(options) = edge_template_json.get("options") {
                ...
                if let Some(edge_type) = options.get("edge_type") {
                    if let Some(values) = edge_type.get("values").and_then(|v| v.as_array()) {
                        ...
                        let app_type_help = edge_type
                            .get("help")
                            .and_then(|h| h.as_object())
                            .unwrap_or(&empty_map);

                        // Create formatted options with descriptions
                        let app_type_display: Vec<String> = app_type_options
                            .iter()
                            .map(|&opt| {
                        ...
                        println!(
                            "Edge computing applications allow you to run Rust code close to your users."
                        );
                        ...
                        let selected_app_type = if let Some(app_type) = application_type {
                            if app_type_options.contains(&app_type) {
                                ...
                                println!(
                                    "Warning: Provided application type '{}' is not valid for edge components",
                                    app_type
                                );
                        ...
                        if let Some(provider_option) = options.get(provider_field) {
                            if let Some(provider_values) =
                                provider_option.get("values").and_then(|v| v.as_array())
                            {
                                let provider_options: Vec<&str> =
                                    provider_values.iter().filter_map(|v| v.as_str()).collect();

                                if provider_options.is_empty() {
                                    return Err(anyhow!(
                                        "No providers found for the selected application type"
                                    ));
                                }

                                // Get the descriptions/help text if available
                                let empty_map = serde_json::Map::new();
                                let provider_help = provider_option
                                    .get("help")
                                    .and_then(|h| h.as_object())
                                    .unwrap_or(&empty_map);

                                // Create formatted options with descriptions
                                let provider_display: Vec<String> = provider_options
                                    .iter()
                                    .map(|&opt| {
                                        if let Some(help) =
                                            provider_help.get(opt).and_then(|h| h.as_str())
                                        {
                                            format!("{} - {}", opt, help)
                                        } else {
                                            opt.to_string()
                                ...
                                vars_map
                                    .insert(provider_field.to_string(), json!(selected_provider));
                                ...
                                if let Some(redirect) = edge_template_json.get("redirect") {
                                    if let Some(app_redirect) = redirect.get(&selected_app_type) {
                                        if let Some(_provider_redirect) =
                                            app_redirect.get(&selected_provider)
                                        {
                                            // We found a redirect configuration, use it to determine the template
                                            template = format!(
                                                "edge/{}/{}",
                                                app_type_clone, provider_clone
                                            );
                                            additional_vars = Some(json!(vars_map));

                                            // Check if the template directory exists
                                            let full_template_path = format!(
                                                "{}/templates/{}",
                                                env!("CARGO_MANIFEST_DIR"),
                                                template
                                            );
                                            ...
                                            return Err(anyhow!(
                                                "No template configuration found for provider: {}",
                                                selected_provider
                                            ));
                                        }
                                    } else {
                                        return Err(anyhow!(
                                            "No template configuration found for application type: {}",
                                            selected_app_type
                                        ));
                                    }
                                } else {
                                    return Err(anyhow!(
                                        "No redirect configuration found in the template"
                                    ));
                                }
                            } else {
                            ...
                            return Err(anyhow!(
                                "No provider options found for the selected application type"
                            ));
                        }
                    } else {
        ...
        if template != "server"
            && template != "serverless"
            && !template.starts_with("data-science/")
        {
    ...
    if template == "client" {
        ...
        let framework_selected = if let Some(fw) = framework {
            if frameworks.contains(&fw) {
                ...
                println!(
                    "Warning: Provided framework '{}' is not valid for client components",
                    fw
                );
        ...
        if framework_selected == "leptos" {
            ...
            if !wasm_output.contains("wasm32-unknown-unknown") {
                ...
                if !status.success() {
                    ...
                    println!(
                        "Please install it manually with: rustup target add wasm32-unknown-unknown"
                    );
                } else {
            ...
            println!(
                "🔧 Creating new Leptos project with {} template...",
                template
            );
            ...
            if let Err(e) =
                templates::apply_template(&template_path, app_path, &name, additional_vars)
            {
                return Err(e);
            }
            ...
            if !ios_output.contains("aarch64-apple-ios-sim") {
                ...
                println!(
                    "This is something that Rustup can do for you. Rustup is Rust's toolchain"
                );
                println!(
                    "installer and manager, and it can add the iOS simulator target to your Rust installation."
                );
                ...
                if install_ios {
                    ...
                    if !status.success() {
                        ...
                        println!(
                            "Please install it manually with: rustup target add aarch64-apple-ios-sim"
                        );
                    } else {
                    ...
                    println!(
                        "You can install it later with: rustup target add aarch64-apple-ios-sim"
                    );
                }
            } else {
            ...
            #[cfg(any(target_os = "macos", target_os = "linux"))]
            {
                ...
                if !android_output.contains("aarch64-linux-android") {
                    ...
                    println!(
                        "For most modern Android devices, you'll want the aarch64-linux-android target."
                    );
                    ...
                    if install_android {
                        ...
                        if !status.success() {
                            ...
                            println!(
                                "Please install it manually with: rustup target add aarch64-linux-android"
                            );
                        } else {
                        ...
                        println!(
                            "You can install it later with: rustup target add aarch64-linux-android"
                        );
                    }
                } else {
            ...
            // Pass arguments directly so the project name is never interpreted by a shell.
            // The destination was created above and verified to be new, so initialize it in place.
            ...
            if !create_status.success() {
                return Err(anyhow!(
                    "Failed to create Tauri project with create-tauri-app"
                ));
            }
        ...
        if let Some(fw) = framework {
            match fw.to_lowercase().as_str() {
                ...
                _ => {
                    println!(
                        "Warning: Provided framework '{}' is not valid for data-science components",
                        fw
                    );
                    ...
                    let framework_options =
                        vec!["Data Analysis with Polars", "Machine Learning with Linfa"];
    ...
    if template == "embedded" {
        ...
        let use_embassy = if let Some(fw) = framework {
            match fw.to_lowercase().as_str() {
                ...
                _ => {
                    println!(
                        "Warning: Provided framework '{}' is not valid for embedded components",
                        fw
                    );
                    ...
                    let frameworks = vec![
                        "No, use standard embedded template",
                        "Yes, use Embassy framework",
                    ];
                    let selection = Select::new()
        ...
        if use_embassy {
            ...
            if is_esp_chip {
                ...
                if !esp_installed {
                    ...
                    if !install_status.success() {
                        ...
                        println!(
                            "See https://esp-rs.github.io/book/installation/index.html for more information."
                        );
                    } else {
            ...
            if project_dir.exists() {
                ...
                if mcu_chip.starts_with("esp") {
                    ...
                    println!(
                        "  For more information: https://esp-rs.github.io/book/installation/index.html"
                    );
                    println!("\n  # Build the project");
                ...
                println!("❌ Failed to create Embassy project.");
                return Err(anyhow!(
                    "Failed to create Embassy project: Project directory not found"
                ));
            }
        } else {
                ...
                if vars.get("mcu_target").is_some() {
                    ...
                    if let Some(ref mut vars_obj) = additional_vars {
                        if let Some(vars_map) = vars_obj.as_object_mut() {
                            vars_map
                                .insert("mcu_target".to_string(), json!(selected_target.clone()));
                        }
                    }
            ...
            match mcu_target.as_str() {
                ...
                "esp32" => {
                    ...
                    println!(
                        "  espflash flash --monitor target/{}/debug/{}",
                        rust_target, name
                    );
                }
                "arduino" => {
                    ...
                    println!(
                        "  1. Install the AVR target:  rustup target add {}",
                        rust_target
                    );
                    println!("  2. Install ravedude:       cargo install ravedude");
                    println!(
                        "  3. Build the project:      cargo build --target {}",
                        rust_target
                    );
                    println!(
                        "  4. Flash to Arduino:       cargo run --target {}",
                        rust_target
                    );
        ...
        // For Leptos templates, prepend "client/leptos/"
        let template_path = format!("client/leptos/{}", template);
        templates::apply_template(&template_path, app_path, &name, additional_vars.clone())?;
    } else {
        ...
        if template.starts_with("data-science/") {
            ...
            templates::apply_template(&template, app_path, &name, Some(json!(template_vars)))?;
        } else {
            // For non-data-science templates, use the original approach
            templates::apply_template(&template, app_path, &name, additional_vars)?;
        }
    }
...
mod tests {
    ...
    fn accepts_safe_project_names() {
        for name in ["demo", "demo-app", "demo_app", "demo2"] {
...
fn check_dependencies(template: &str) -> Result<()> {
    ...
    if template.starts_with("edge/") {
        ...
        if template.contains("cloudflare-workers") || template.contains("cloudflare-pages") {
            ...
            match wrangler_check {
                ...
                Err(_) => {
                    ...
                    println!(
                        "Learn more at: https://developers.cloudflare.com/workers/wrangler/install-and-update/"
                    );
                }
            }
        }
        ...
        if template.contains("vercel") {
            ...
            let vercel_check = Command::new("vercel").arg("--version").output();
        ...
        if template.contains("netlify") {
            ...
            let netlify_check = Command::new("netlify").arg("--version").output();
        ...
        if template.contains("fastly") {
            ...
            let fastly_check = Command::new("fastly").arg("--version").output();
            ...
            match fastly_check {
                ...
                Err(_) => {
                    ...
                    println!(
                        "You may need to install it from: https://developer.fastly.com/learning/tools/cli/"
                    );
                }
            }
        }
...
fn handle_edge_template(
    template: &str,
    app_path: &Path,
    name: &str,
    additional_vars: Option<serde_json::Value>,
) -> Result<()> {
    // Handle edge template creation manually
    let template_dir_path = PathBuf::from(format!(
        "{}/templates/{}",
        env!("CARGO_MANIFEST_DIR"),
        template
    ));
    ...
    if !template_dir_path.exists() {
        return Err(anyhow::anyhow!(
            "Could not find template directory for {} template",
            template
        ));
    }
    ...
    let cargo_toml_path = if cargo_toml_template.exists() {
        ...
        return Err(anyhow::anyhow!(
            "Could not find Cargo.toml or Cargo.toml.template for {} template",
            template
        ));
    };
    ...
    if template_json_path.exists() {
        ...
        if let Ok(template_config) =
            serde_json::from_str::<serde_json::Value>(&template_json_content)
        {
