...
"#
    ...
    match cli.command {
        ...
        Some(commands::Commands::Transform { project, template }) => {
            match &project {
                Some(p) => println!("{} {}", "Transforming".yellow().bold(), p.cyan().bold()),
                None => println!(
                    "{}",
                    "Starting interactive project transformation"
                        .yellow()
                        .bold()
                ),
            }
        ...
        Some(commands::Commands::Preview {
            component_type,
            framework,
            provider,
            application_type,
            ...
            commands::preview::execute(
                component_type_str.as_deref(),
                framework.as_deref(),
                provider.as_deref(),
                application_type.as_deref(),
            )
        ...
        #[cfg(not(feature = "workspace_test"))]
        Some(commands::Commands::Component {
            action,
            component_type,
            project,
        }) => {
            ...
            commands::component::execute(action.as_deref(), component_type_ref, project.as_deref())
        }
        #[cfg(not(feature = "workspace_test"))]
        Some(commands::Commands::Config {
            export,
            import,
            path,
        }) => {
        ...
        Some(commands::Commands::UnusedFeatures { path }) => {
            println!(
                "{}",
                "Finding unused features in dependencies".green().bold()
            );
        ...
        None => {
            println!(
                "{}",
                "No command specified, using interactive mode".yellow()
            );
