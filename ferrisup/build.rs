...
fn main() {
    ...
    println!(
        "cargo:rustc-env=FERRISUP_TEMPLATES_DIR={}",
        out_path.display()
    );
}
...
fn copy_templates_dir<P: AsRef<Path>>(templates_dir: &str, out_dir: P) {
    ...
    fs::create_dir_all(&target_dir)
        .unwrap_or_else(|_| panic!("Failed to create templates directory"));
...
fn copy_dir_recursively(source: &Path, target: &Path) {
    ...
    for entry in
        fs::read_dir(source).unwrap_or_else(|_| panic!("Failed to read directory: {:?}", source))
    {
