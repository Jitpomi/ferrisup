...
fn test_ferrisup_version() -> Result<()> {
    ...
    assert!(
        std::process::Command::new(env!("CARGO_BIN_EXE_ferrisup"))
            .arg("--version")
            .status()?
            .success()
    );
...
fn test_cargo_metadata() -> Result<()> {
    ...
    assert!(
        ...
        "Repository URL should be provided"
    );
    ...
    assert!(
        cargo_toml_content.contains("keywords = ["),
...
fn test_all_templates() -> Result<()> {
    ...
    let template_tuples = ferrisup::project::templates::list_templates()?;
    ...
    for (template_name, _template_description) in &template_tuples {
        ...
        if !output.status.success() {
            ...
            if src_dir.exists() {
                ...
                assert!(
                    cargo_content.contains(&project_name),
    ...
    assert!(
        success_count > 0,
        "At least one template should create successfully"
    );
...
fn test_readme_contains_required_sections() -> Result<()> {
    ...
    assert!(
        ...
        "README should have license information"
    );
