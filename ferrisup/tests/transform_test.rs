...
fn test_error_handling_in_utils() -> Result<()> {
    ...
    assert!(
        error.contains("Cargo.toml not found"),
...
fn test_error_handling_with_nonexistent_directory() {
    ...
    let error = result.unwrap_err().to_string();
    assert!(
        error.contains("Failed to create directory") || error.contains("No such file or directory"),
...
#[test]
#[ignore = "Transform currently enters the interactive component flow before validating this template"]
fn test_transform_execute_with_invalid_template() -> Result<()> {
    ...
    ferrisup::commands::test_mode::enable_test_mode();
    ...
    let error = result.unwrap_err().to_string();
    assert!(
        error.contains("Invalid template") || error.contains("Unknown template"),
...
fn test_transform_execute_with_invalid_path() -> Result<()> {
    ...
    let result = ferrisup::commands::transform::execute(Some(invalid_path), Some("library"));
