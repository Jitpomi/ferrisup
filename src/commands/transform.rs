...
fn convert_to_workspace(project_dir: &Path) -> Result<()> {
    ...
    let component_name = Input::<String>::new()
        .with_prompt(format!("What would you like to name the first component? [{}]", default_name))
...
fn add_component(project_dir: &Path) -> Result<()> {
    ...
    if component_cargo_path.exists() {
        ...
        println!("{}", format!("Using project name '{}' for imports", actual_project_name).blue());
...
fn add_component_without_workspace(project_dir: &Path) -> Result<()> {
    ...
    if component_cargo_path.exists() {
        ...
        // Get project name from directory name if not available from structure
        // First check if there's a Cargo.toml with a package name
        let project_cargo_path = project_dir.join("Cargo.toml");
        let project_name = if project_cargo_path.exists() {
            if let Ok(cargo_content) = fs::read_to_string(&project_cargo_path) {
                if let Ok(cargo_doc) = cargo_content.parse::<Document>() {
        ...
        println!("{}", format!("Using project name '{}' for imports", project_name).blue());
